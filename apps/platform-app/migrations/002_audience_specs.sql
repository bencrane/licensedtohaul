...
CREATE INDEX audience_specs_partner_idx
  ON audience_specs (partner_org_id, status, updated_at DESC);
...
DROP TRIGGER IF EXISTS audience_specs_touch_updated_at ON audience_specs;
CREATE TRIGGER audience_specs_touch_updated_at
  BEFORE UPDATE ON audience_specs
  FOR EACH ROW
...
ALTER TABLE transfers
  ...
  FOREIGN KEY (audience_spec_id) REFERENCES audience_specs(id) ON DELETE SET NULL;
