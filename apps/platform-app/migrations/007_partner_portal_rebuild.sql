...
-- Forward-only. Per validator: text FKs throughout (no organizations dependency).
...
CREATE TABLE IF NOT EXISTS deal_room_messages (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  thread_id uuid NOT NULL REFERENCES deal_room_threads(id) ON DELETE CASCADE,
  sender_side text NOT NULL CHECK (sender_side IN ('carrier','partner')),
  body text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_deal_room_messages_thread ON deal_room_messages (thread_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_disbursements_factor_quarter
  ON disbursements (factor_slug, disbursed_at);

-- Per-factor billing config (overrides defaults)
CREATE TABLE IF NOT EXISTS factor_billing_config (
...
ALTER TABLE factor_stripe_customers
...
DROP TABLE IF EXISTS audience_specs CASCADE;
DROP TABLE IF EXISTS message_threads CASCADE;

-- Mark transfers as deprecated (conditional — table may not exist in test schemas)
DO $$
BEGIN
  IF EXISTS (
