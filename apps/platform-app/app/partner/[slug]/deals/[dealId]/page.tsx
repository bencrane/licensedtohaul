...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: t
      ? `${t.contact_snapshot.name} · deal — Licensed to Haul`
      : "Deal — Licensed to Haul",
  };
}

export default async function DealDetailPage({ params }: Props) {
  const { slug, dealId } = await params;
  if (!slug || !dealId) notFound();

  const deal = await getTransferById(dealId);
  if (!deal) notFound();

  const messages = await listMessagesForTransfer(dealId);
  const cs = deal.contact_snapshot;
            ...
            <section className="border border-line bg-surface">
              ...
              <div className="grid grid-cols-5 gap-1 p-2">
                {DISPOSITION_STEPS.map((step) => {
                  const active = deal.disposition === step.key;
