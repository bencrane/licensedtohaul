...
export async function getOrCreateThread(
  ...
  const { rows } = await db.query<DealRoomThread>(
    `INSERT INTO deal_room_threads (factor_slug, carrier_dot)
     VALUES ($1, $2)
...
export async function insertMessage(
  ...
  const { rows } = await db.query<DealRoomMessage>(
    `INSERT INTO deal_room_messages (thread_id, sender_side, body)
     VALUES ($1, $2, $3)
...
export async function getMessages(
  ...
  const { rows } = await db.query<DealRoomMessage>(
    `SELECT m.id, m.thread_id, m.sender_side, m.body, m.created_at
     ...
     JOIN deal_room_threads t ON t.id = m.thread_id
