...
export default function MessageThread({
  ...
  const action = sendMessage.bind(null, slug, transferId);
  ...
  // Clear textarea after a successful send and scroll new message into view.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col border border-line bg-surface">
      ...
      <ul className="flex max-h-[480px] min-h-[280px] flex-col gap-3 overflow-y-auto px-5 py-5">
        ...
        {messages.map((m) => {
          const isPartner = m.sender_side === 'partner';
              ...
              >
                ...
                <p
                  className={`mt-1 text-[10px] font-mono uppercase tracking-[0.12em] ${
                    isPartner ? 'text-orange-100' : 'text-stone-400'
                  }`}
                >
                  {isSystem ? 'system' : isPartner ? 'you' : carrierName} ·{' '}
      ...
      >
        <textarea
          ...
          required
          rows={3}
          placeholder={`Write to ${carrierName}…`}
        ...
        <div className="flex items-center justify-between">
          <p className="text-[11px] text-stone-500">
            Carrier receives this as an email. Replies will surface here.
          </p>
          <button
            type="submit"
            disabled={pending}
